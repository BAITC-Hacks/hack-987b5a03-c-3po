...
const translations: Record<string, [string, string]> = {
  ...
  "Custom dataset": ["Свои данные", "Өз деректеріңіз"],
  "Analyze your own data": [
    "Проанализируйте свои данные",
    "Өз деректеріңізді талдаңыз",
  ],
  "Required columns: src, dst, date, sum_kzt": [
    "Обязательные колонки: src, dst, date, sum_kzt",
    "Міндетті бағандар: src, dst, date, sum_kzt",
  ],
  "Choose CSV files": ["Выберите CSV-файлы", "CSV файлдарын таңдаңыз"],
  "or drop them here · up to 12 files / 25 MB": [
    "или перетащите сюда · до 12 файлов / 25 МБ",
    "немесе осында сүйреңіз · 12 файлға / 25 МБ дейін",
  ],
  "file selected": ["файл выбран", "файл таңдалды"],
  "files selected": ["файлов выбрано", "файл таңдалды"],
  "Seed client GIDs": ["GID исходных клиентов", "Бастапқы клиенттердің GID-і"],
  "e.g. 9007199254740993, 9007199254740995": [
    "например: 9007199254740993, 9007199254740995",
    "мысалы: 9007199254740993, 9007199254740995",
  ],
  "Seed GIDs": ["Исходные GID", "Бастапқы GID"],
  "Separate IDs with commas, spaces, or new lines": [
    "Разделяйте ID запятыми, пробелами или переносами строк",
    "ID мәндерін үтірмен, бос орынмен немесе жаңа жолмен бөліңіз",
  ],
  "Files stay in this local workspace": [
    "Файлы остаются в локальном рабочем пространстве",
    "Файлдар жергілікті жұмыс кеңістігінде қалады",
  ],
  "Prepare dataset": ["Подготовить данные", "Деректерді дайындау"],
  "Dataset ready": ["Данные готовы", "Деректер дайын"],
  files: ["файлов", "файл"],
  "Run started": ["Анализ запущен", "Талдау басталды"],
  "Start bundled demo": ["Запустить демо", "Демоны бастау"],
  "Start OpenAI run": ["Запустить через OpenAI", "OpenAI арқылы іске қосу"],
  "Reset demo": ["Сбросить демо", "Демоны қалпына келтіру"],
  "New run": ["Новый запуск", "Жаңа іске қосу"],
  "Agent provider mode": ["Режим провайдера агента", "Агент провайдерінің режимі"],
  Demo: ["Демо", "Демо"],
  "OpenAI live": ["OpenAI live", "OpenAI live"],
  "Use the configured OpenAI provider": [
    "Использовать настроенный провайдер OpenAI",
    "Бапталған OpenAI провайдерін пайдалану",
  ],
  "Configure OPENAI_API_KEY to enable live mode": [
    "Настройте OPENAI_API_KEY, чтобы включить live mode",
    "Live mode қосу үшін OPENAI_API_KEY баптаңыз",
  ],
  "OpenAI chooses tools; analytics and verification remain deterministic": [
    "OpenAI выбирает инструменты; анализ и проверка остаются детерминированными",
    "OpenAI құралдарды таңдайды; талдау мен тексеру детерминирленген күйде қалады",
  ],
  "Add OPENAI_API_KEY to the ignored .env file to enable live mode": [
    "Добавьте OPENAI_API_KEY в исключённый из Git файл .env, чтобы включить live mode",
    "Live mode қосу үшін Git елемейтін .env файлына OPENAI_API_KEY қосыңыз",
  ],
  ...
  "Excel review report": [
    "Отчёт Excel для проверки",
    "Тексеруге арналған Excel есебі",
  ],
  "Formatted tables for manual review": [
    "Форматированные таблицы для ручной проверки",
    "Қолмен тексеруге арналған пішімделген кестелер",
  ],
